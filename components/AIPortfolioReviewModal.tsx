    ...
    >
      ...
      >
        ...
        <div className="flex-grow overflow-y-auto p-6 space-y-6">
            {isLoading && (
                <div className="flex flex-col items-center justify-center p-16 text-center">
                    ...
                    <p className="font-semibold text-slate-700 dark:text-slate-300">{t('aiAssistant.generating')}</p>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">{t('aiAssistant.generating.message')}</p>
                </div>
            )}
            ...
            {review && (
                <div className="space-y-6">
                    <FeedbackSection title={t('aiAssistant.overallImpression')} icon={Lightbulb}>
                        <p>{review.overallImpression}</p>
                    </FeedbackSection>
                    
                    <FeedbackSection title={t('aiAssistant.contentSuggestions')} icon={Edit3}>
                     ...
                     <FeedbackSection title={t('aiAssistant.projectShowcase')} icon={FolderKanban}>
                        <p>{review.projectShowcaseFeedback}</p>
                    </FeedbackSection>
                    
                    {review.missingSections.length > 0 && (
                        <FeedbackSection title={t('aiAssistant.missingSections')} icon={PlusCircle}>
        ...
        <footer className="flex-shrink-0 p-4 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-3 bg-slate-50 dark:bg-slate-800/50 rounded-b-lg">
          <Button variant="primary" onClick={onClose}>{t('aiAssistant.done')}</Button>
        </footer>
      </motion.div>
    </motion.div>
  );
