...
const CreateResumeModal: React.FC<CreateResumeModalProps> = ({ onClose, onCreated }) => {
  ...
  const { user, portfolios, createResume, projects, skills, consumeAiFeature } = useData();
      ...
      >
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setMode('ai')} className={`p-4 rounded-lg border-2 text-left transition-colors ${mode === 'ai' ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/30' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600'}`}>
                    <Sparkles size={20} className="mb-2 text-teal-500"/>
                    <h4 className="font-semibold text-slate-800 dark:text-slate-200">{t('generateFromPortfolio')}</h4>
                </button>
                <button onClick={() => setMode('scratch')} className={`p-4 rounded-lg border-2 text-left transition-colors ${mode === 'scratch' ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/30' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600'}`}>
                    <FilePlus size={20} className="mb-2 text-teal-500"/>
                    <h4 className="font-semibold text-slate-800 dark:text-slate-200">{t('createFromScratch')}</h4>
                </button>
            </div>
            {mode === 'ai' && (
                <div>
